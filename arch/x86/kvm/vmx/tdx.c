...
static void tdx_tdpart_cleanup(struct kvm_tdx *kvm_tdx)
{
	...
	for (i = 0; i < TDX_MAX_L2_VMS; i++) {
		...
		list_for_each_entry_safe(p, n, head, node) {
			l2sept_hpa = p->hpa;
			if (WARN_ON(tdx_reclaim_page(l2sept_hpa, false, 0)))
				continue;
			list_del(&p->node);
...
void tdx_load_mmu_pgd(struct kvm_vcpu *vcpu, hpa_t root_hpa, int pgd_level)
{
	...
	for (i = 0; i < kvm_tdx->num_l2_vms; i++)
		l2td_vmcs_write64(to_tdx(vcpu), SHARED_EPT_POINTER,
				  root_hpa & PAGE_MASK, i);
}
...
free:
	...
	trace_kvm_tdx_page_demote(kvm_tdx->tdr_pa, gfn, hpa >> PAGE_SHIFT, level, ret);
...
static int tdx_sept_remove(struct kvm *kvm, gpa_t gpa, int level, void *l1sept_page)
{
	struct kvm_tdx *kvm_tdx = to_kvm_tdx(kvm);
	...
	if (!kvm_tdx->num_l2_vms)
		err = tdh_mem_sept_remove_v0(kvm_tdx->tdr_pa, gpa, parent_tdx_level, &out);
	else
		err = tdh_mem_sept_remove_v1(kvm_tdx->tdr_pa, gpa, parent_tdx_level, &out);
	...
	tdx_set_page_present(l1sept_hpa);
	tdx_clear_page(l1sept_hpa, PAGE_SIZE);
				     ...
				     enum pg_level level, void *private_spt)
{
	...
	r = tdx_sept_remove(kvm, parent_gpa, level, private_spt);
	if (!r)
		trace_kvm_tdx_sept_remove(kvm_tdx->tdr_pa, gfn,
				__pa(private_spt) >> PAGE_SHIFT, level);
...
static int tdx_module_setup(void)
{
	...
	/*
	 * With L2 VM supported, nr_tdcs_pages and nr_tdvpx_pages need
	 * to be extended to support L2 VMs when add pages.
	 */
	if (tdx_info.max_num_l2_vms) {
		tdx_info.nr_tdcs_pages += tdx_info.max_num_l2_vms * TDX_TDCS_PAGES_PER_L2;
		tdx_info.nr_tdvpx_pages += tdx_info.max_num_l2_vms * TDX_TDVPS_PAGES_PER_L2;
	}
